...
function init(
    ...
    chain = Models.build_chain(config.arch, embed_chain; ins, outsize, loss)
    ...
    data = Models.train_dataloader(
        config.arch, m, data, df;
        feature_names, target_name, weight_name, offset_name, group_name,
        loss, scalers, batchsize, dev, rng, backend=config.backend)
