
# Getting started with NeuroTabModels.jl {#Getting-started-with-NeuroTabModels.jl}

## Installation {#Installation}

```julia
] add NeuroTabModels
```


## Configuring a model {#Configuring-a-model}

A model configuration is defined with the [NeuroTabRegressor](/models#NeuroTabRegressor) constructor:

```julia
using NeuroTabModels, DataFrames

config = NeuroTabRegressor(
    loss = :mse,
    nrounds = 10,
    num_trees = 16,
    depth = 5,
)
```


## Training {#Training}

Building and training a model according to the above `config` is done with [NeuroTabModels.fit](/API#MLJModelInterface.fit). See the docs for additional features, notably early stopping support through the tracking of an evaluation metric.

```julia
nobs, nfeats = 1_000, 5
dtrain = DataFrame(randn(nobs, nfeats), :auto)
dtrain.y = rand(nobs)
feature_names, target_name = names(dtrain, r"x"), "y"

m = NeuroTabModels.fit(config, dtrain; feature_names, target_name)
```


## Inference {#Inference}

```julia
p = m(dtrain)
```


## MLJ {#MLJ}

NeuroTabModels.jl supports the [MLJ](https://github.com/alan-turing-institute/MLJ.jl) Interface. 

```julia
using MLJBase, NeuroTabModels
m = NeuroTabRegressor(depth=5, nrounds=10)
X, y = @load_boston
mach = machine(m, X, y) |> fit!
p = predict(mach, X)
```


## Benchmarks {#Benchmarks}

Benchmarking against prominent ML libraries for tabular is performed at [MLBenchmarks.jl](https://github.com/Evovest/MLBenchmarks.jl).
