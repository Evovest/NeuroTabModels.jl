...
# Logistic Regression on Titanic Dataset {#Logistic-Regression-on-Titanic-Dataset}
...
Now we are ready to train our model. We first define a model configuration using the [`NeuroTabRegressor`](/models#NeuroTabRegressor) model constructor.  Then, we use [`NeuroTabModels.fit`](/API#MLJModelInterface.fit) to train a boosted tree model. We pass the optional `deval` argument to enable the usage of early stopping. 
